...
const blockquoteSlotNames = [
	...
	['icon', 'blockquote__icon'],
	['content', 'blockquote__content'],
	['caption', 'blockquote__caption'],
]
const blockquoteSlotFns = /* @__PURE__ */ blockquoteSlotNames.map(
	([slotName, slotKey]) => [
		slotName,
		createRecipe(
			slotKey,
			blockquoteDefaultVariants,
			getSlotCompoundVariant(blockquoteCompoundVariants, slotName),
		),
	],
)
...
const blockquoteFn = memo((props = {}) => {
	return Object.fromEntries(
		blockquoteSlotFns.map(([slotName, slotFn]) => [
			slotName,
...
const blockquoteVariantKeys = ['justify', 'variant']
