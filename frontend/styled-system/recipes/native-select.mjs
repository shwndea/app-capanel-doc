...
const nativeSelectSlotNames = [
	...
	['field', 'native-select__field'],
	['indicator', 'native-select__indicator'],
]
const nativeSelectSlotFns = /* @__PURE__ */ nativeSelectSlotNames.map(
	([slotName, slotKey]) => [
		slotName,
		createRecipe(
			slotKey,
			nativeSelectDefaultVariants,
			getSlotCompoundVariant(nativeSelectCompoundVariants, slotName),
		),
	],
)
...
const nativeSelectFn = memo((props = {}) => {
	return Object.fromEntries(
		nativeSelectSlotFns.map(([slotName, slotFn]) => [
			slotName,
...
const nativeSelectVariantKeys = ['variant', 'size']
