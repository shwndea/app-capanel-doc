...
const stepsSlotNames = [
	...
	['title', 'steps__title'],
	['description', 'steps__description'],
	['nextTrigger', 'steps__nextTrigger'],
	['prevTrigger', 'steps__prevTrigger'],
	['progress', 'steps__progress'],
]
const stepsSlotFns = /* @__PURE__ */ stepsSlotNames.map(
	([slotName, slotKey]) => [
		slotName,
		createRecipe(
			slotKey,
			stepsDefaultVariants,
			getSlotCompoundVariant(stepsCompoundVariants, slotName),
		),
	],
)
...
const stepsFn = memo((props = {}) => {
	return Object.fromEntries(
		stepsSlotFns.map(([slotName, slotFn]) => [
			slotName,
...
const stepsVariantKeys = ['orientation', 'variant', 'size']
